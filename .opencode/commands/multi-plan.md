...
> *"This command requires `~/.claude/bin/codeagent-wrapper` and the `ace-tool` MCP server. These are not part of the standard clarc setup. Use `/plan` instead for a native Claude planning workflow."*
