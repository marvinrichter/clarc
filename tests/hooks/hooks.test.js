...
async function runTests() {
        ...
        for (const entry of hookArray) {
          for (const hook of entry.hooks) {
            if (hook.type === 'command') {
              ...
              assert.ok(isNode || isSkillScript, `Hook command should start with 'node' or be a skill shell script: ${hook.command.substring(0, 80)}...`);
            }
          }
        }
      };
  ...
  // ── Round 53: format non-existent file ──
...
`;
  ...
  // ── Round 95: pre-bash-dispatch.js ──
  console.log('\nRound 95: pre-bash-dispatch.js:');

  if (
    await asyncTest('passes through on empty stdin', async () => {
      const result = await runScript(path.join(scriptsDir, 'pre-bash-dispatch.js'), '');
      assert.strictEqual(result.code, 0, 'Should exit 0 on empty stdin');
    })
  )
    passed++;
  ...
  if (
    await asyncTest('passes through on invalid JSON', async () => {
      const result = await runScript(path.join(scriptsDir, 'pre-bash-dispatch.js'), 'not json');
      assert.strictEqual(result.code, 0, 'Should exit 0 on invalid JSON');
    })
  )
    passed++;
  ...
  if (
    await asyncTest('passes through non-dev-server command', async () => {
      const input = JSON.stringify({ tool_input: { command: 'git status' } });
      const result = await runScript(path.join(scriptsDir, 'pre-bash-dispatch.js'), input);
      assert.strictEqual(result.code, 0, 'Should exit 0 for safe commands');
      assert.ok(result.stdout.includes('git status'), 'Should pass stdin through to stdout');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('blocks dev server outside tmux (exit 2)', async () => {
      const input = JSON.stringify({ tool_input: { command: 'npm run dev' } });
      const result = await runScript(path.join(scriptsDir, 'pre-bash-dispatch.js'), input, { TMUX: '', CLAUDE_PLUGIN_ROOT: path.join(__dirname, '..', '..') });
      // Only blocks on non-Windows; on macOS/Linux expect exit 2 or 0 (if TMUX is set)
      if (process.platform !== 'win32') {
        assert.ok(result.code === 2 || result.code === 0, 'Should block (2) or pass (0 if in tmux)');
      }
    })
  )
    passed++;
  ...
  // ── Round 96: post-edit-format-dispatch.js ──
  console.log('\nRound 96: post-edit-format-dispatch.js:');

  if (
    await asyncTest('passes through on empty stdin', async () => {
      const result = await runScript(path.join(scriptsDir, 'post-edit-format-dispatch.js'), '');
      assert.strictEqual(result.code, 0, 'Should exit 0');
    })
  )
    passed++;
  ...
  if (
    await asyncTest('passes through on unknown extension', async () => {
      const input = JSON.stringify({ tool_input: { file_path: '/foo/bar.xyz' } });
      const result = await runScript(path.join(scriptsDir, 'post-edit-format-dispatch.js'), input, { CLAUDE_PLUGIN_ROOT: path.join(__dirname, '..', '..') });
      assert.strictEqual(result.code, 0, 'Should exit 0 for unknown extensions');
      assert.ok(result.stdout.includes('bar.xyz'), 'Should pass stdin through');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('warns on console.log in JS file (inline check)', async () => {
      const testDir = createTestDir();
      const jsFile = path.join(testDir, 'test.js');
      fs.writeFileSync(jsFile, 'function foo() { console.log("debug"); return 42; }');
      const input = JSON.stringify({ tool_input: { file_path: jsFile } });
      const result = await runScript(path.join(scriptsDir, 'post-edit-format-dispatch.js'), input, { CLAUDE_PLUGIN_ROOT: path.join(__dirname, '..', '..') });
      assert.ok(result.stderr.includes('console.log') || result.code === 0, 'Should warn about console.log or pass through');
      cleanupTestDir(testDir);
    })
  )
    passed++;
  ...
  // ── Round 97: post-edit-typecheck-rust.js ──
  console.log('\nRound 97: post-edit-typecheck-rust.js:');

  if (
    await asyncTest('passes through on empty stdin', async () => {
      const result = await runScript(path.join(scriptsDir, 'post-edit-typecheck-rust.js'), '');
      assert.strictEqual(result.code, 0, 'Should exit 0');
    })
  )
    passed++;
  ...
  if (
    await asyncTest('passes through on non-rust file', async () => {
      const input = JSON.stringify({ tool_input: { file_path: '/foo/bar.ts' } });
      const result = await runScript(path.join(scriptsDir, 'post-edit-typecheck-rust.js'), input);
      assert.strictEqual(result.code, 0, 'Should exit 0 for non-.rs files');
      assert.ok(result.stdout.includes('bar.ts'), 'Should pass stdin through');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('passes through on .rs file with no Cargo.toml', async () => {
      const testDir = createTestDir();
      const rsFile = path.join(testDir, 'main.rs');
      fs.writeFileSync(rsFile, 'fn main() {}');
      const input = JSON.stringify({ tool_input: { file_path: rsFile } });
      const result = await runScript(path.join(scriptsDir, 'post-edit-typecheck-rust.js'), input);
      assert.strictEqual(result.code, 0, 'Should exit 0 when no Cargo.toml found');
      cleanupTestDir(testDir);
    })
  )
    passed++;
  ...
  if (
    await asyncTest('logHook does not throw on valid args', async () => {
      ...
      try {
        assert.doesNotThrow(() => logHook('test-hook', 'Edit', '/foo/bar.ts', 0, 42));
      } finally {
  ...
  // ── Round 99: notification-handler.js ──
  console.log('\nRound 99: notification-handler.js:');

  if (
    await asyncTest('passes through on empty stdin', async () => {
      const result = await runScript(path.join(scriptsDir, 'notification-handler.js'), '');
      assert.strictEqual(result.code, 0, 'Should exit 0');
    })
  )
    passed++;
  ...
  if (
    await asyncTest('handles unknown notification type silently', async () => {
      const input = JSON.stringify({ notification: { type: 'unknown_type' } });
      const result = await runScript(path.join(scriptsDir, 'notification-handler.js'), input);
      assert.strictEqual(result.code, 0, 'Should exit 0 for unknown notification types');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('emits compact suggestion on context_limit_warning', async () => {
      const input = JSON.stringify({ notification: { type: 'context_limit_warning' } });
      const result = await runScript(path.join(scriptsDir, 'notification-handler.js'), input);
      assert.strictEqual(result.code, 0, 'Should exit 0');
      assert.ok(result.stdout.includes('compact') || result.stdout.includes('context'), 'Should mention compact or context in output');
    })
  )
    passed++;
  else failed++;
