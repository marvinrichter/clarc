...
function runTests() {
  ...
  const stdinScript = 'import("./scripts/lib/utils.js").then((u)=>u.readStdinJson({timeoutMs:2000})).then(d=>{process.stdout.write(JSON.stringify(d))})';
  const stdinOpts = { encoding: 'utf8', cwd: path.join(__dirname, '..', '..'), timeout: 5000 };

  if (
    test('readStdinJson parses valid JSON from stdin', () => {
      const result = execFileSync('node', ['--input-type=module', '-e', stdinScript], { ...stdinOpts, input: '{"tool_input":{"command":"ls"}}' });
  ...
  if (
    test('readStdinJson returns {} for invalid JSON', () => {
      const result = execFileSync('node', ['--input-type=module', '-e', stdinScript], { ...stdinOpts, input: 'not json' });
  ...
  if (
    test('readStdinJson returns {} for empty stdin', () => {
      const result = execFileSync('node', ['--input-type=module', '-e', stdinScript], { ...stdinOpts, input: '' });
      assert.deepStrictEqual(JSON.parse(result), {});
    })
  )
    passed++;
  ...
  if (
    test('readStdinJson handles nested objects', () => {
      const result = execFileSync('node', ['--input-type=module', '-e', stdinScript], { ...stdinOpts, input: '{"a":{"b":1},"c":[1,2]}' });
      const parsed = JSON.parse(result);
  ...
  if (
    test('readStdinJson maxSize stops accumulating after threshold (chunk-level guard)', () => {
      ...
      const script = 'import("./scripts/lib/utils.js").then((u)=>u.readStdinJson({timeoutMs:2000,maxSize:100})).then(d=>{process.stdout.write(JSON.stringify(d))})';
      // Generate 100KB of data (arrives in multiple chunks)
      const bigInput = '{"k":"' + 'X'.repeat(100000) + '"}';
      const result = execFileSync('node', ['--input-type=module', '-e', script], { ...stdinOpts, input: bigInput });
  ...
  if (
    test('readStdinJson with maxSize large enough preserves valid JSON', () => {
      const script = 'import("./scripts/lib/utils.js").then((u)=>u.readStdinJson({timeoutMs:2000,maxSize:1024})).then(d=>{process.stdout.write(JSON.stringify(d))})';
      const input = JSON.stringify({ key: 'value' });
      const result = execFileSync('node', ['--input-type=module', '-e', script], { ...stdinOpts, input });
  ...
  if (
    test('readStdinJson handles JSON with trailing whitespace/newlines', () => {
      const result = execFileSync('node', ['--input-type=module', '-e', stdinScript], { ...stdinOpts, input: '{"a":1}  \n\n' });
  ...
  if (
    test('readStdinJson resolves {} when stdin emits error (via broken pipe)', () => {
      ...
      const script = 'import("./scripts/lib/utils.js").then((u)=>u.readStdinJson({timeoutMs:2000})).then(d=>{process.stdout.write(JSON.stringify(d))})';
  ...
  if (
    test('getSessionIdShort uses fallback when getProjectName returns null (CWD at root)', () => {
      ...
      const script = `import(${JSON.stringify(utilsPath)}).then((utils)=>{process.stdout.write(utils.getSessionIdShort('my-custom-fallback'))})`;
